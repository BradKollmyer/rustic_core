#!/usr/bin/env python3
"""Profile four cold-cache S3 restores at 5/10/10/5 connections.

Creates and checks one disposable source fixture, verifies every restored byte,
and deletes only its unique prefix in the dedicated rustic-prune-bench bucket.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    prefix = 'bench-restore-'+str(uuid.uuid4())+'/'
    binary = args.binary.resolve()
    manifest = dict(endpoint=credentials['CEPH_FURIES_S3_ENDPOINT'], bucket=bucket, prefix=prefix,
        source_mib=args.size_mib, file_mib=16, pack_mib=128, backend_connections=[5, 10, 10, 5],
    ...
    env = dict(os.environ, **credentials, PRUNE_S3_ROOT=prefix, RESTORE_S3_MIB=str(args.size_mib))
    def limits():
    ...
    try:
        ...
        proc = subprocess.Popen([str(binary), 'benchmark_restore_s3', '--exact', '--ignored', '--nocapture'],
        ...
        def read():
            with (output/'run.log').open('w') as log:
                for line in proc.stdout:
                    ...
                    log.write(line)
                    log.flush()
                    if line.startswith('BEGIN restore-'):
                        name = line.strip().removeprefix('BEGIN restore-')
                        ...
                        phase['name'] = None
                        print(line.strip(), flush=True)
                    elif line.startswith(('CHECK ', 'SETUP ')):
                        print(line.strip(), flush=True)
        ...
        while proc.poll() is None:
            if time.monotonic()-start > 1800:
                raise TimeoutError('Restore test exceeded 30 minutes')
            name = phase['name']
            if name:
                if args.profile and name not in profiled and time.monotonic()-windows[name]['start'] >= 5:
            ...
            time.sleep(.5)
        reader.join()
        if proc.returncode or len(results)!=4 or len([l for l in lines if l.startswith('CHECK ')])!=4:
            raise RuntimeError('Restore/verification failed; see run.log')
        for result in results:
            ...
            result['restore_mibps'] = result['source_mib']/result['seconds']
            result['transfer_mibps'] = result['source_mib']/result['transfer_seconds']
