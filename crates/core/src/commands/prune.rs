...
pub(crate) fn prune_repository<S: Open>(
    ...
    // Estimate fixed repack targets from retained blob bytes and pack headers.
    // Recompression and the final blob can change the actual output size.
    ...
    if connections.is_some() {
        for blob_type in [BlobType::Tree, BlobType::Data] {
            let stats = &prune_plan.stats.size[blob_type];
            let required =
                u64::from(pack_sizer[blob_type].pack_size()).min(stats.repack - stats.repackrm);
            if required > opts.repack_upload_buffer.as_u64() {
                return Err(RusticError::new(ErrorKind::InvalidInput,
                    "The planned repack size exceeds the upload buffer; increase --repack-upload-buffer.")
                    .attach_context("option", "--repack-upload-buffer")
                    .attach_context("required_bytes", required.to_string())
                    .attach_context("budget_bytes", opts.repack_upload_buffer.as_u64().to_string()));
            }
        }
    }
    ...
    let indexes_remove: Vec<_> = prune_plan
        .index_files
        .iter()
        .map(|index| index.id)
        .collect();
...
mod used_id_map_tests {
    ...
    #[test]
    fn loader_records_unique_ids_and_merges_across_maps() {
        let (tx, rx) = mpsc::channel();
        let mut loader = UsedIdAcc {
            map: UsedIdMap::default(),
            tx,
        };
        ...
        loader.on_load(&tree);
        loader.finish();
        let mut ids = rx.recv().unwrap();
        assert_eq!(ids.len(), 2);
        assert_eq!(ids.get(&blob(ID_A)).copied(), Some(0));
        assert_eq!(ids.get(&blob(ID_C)).copied(), Some(0));

        let mut other = UsedIdMap::default();
        _ = other.insert(blob(ID_B), 0);
        _ = other.insert(blob(ID_C), 0);
        ids.extend(other);
        assert_eq!(ids.len(), 3);
        assert_eq!(ids.remove(&blob(ID_B)), Some(0));
        assert!(ids.get(&blob(ID_B)).is_none());
        assert_eq!(ids.len(), 2);
    }
}
...
mod marked_pack_compaction_tests {
    ...
    fn pack(used: u32, unused: u32, time: Option<Timestamp>) -> (IndexPack, UsedIdMap) {
        let mut pack = IndexPack {
            id: PackId::from(crate::Id::random()),
            time,
            ..IndexPack::default()
        };
        let mut ids = UsedIdMap::default();
        let mut offset = 0;
    ...
    fn plan(
        ...
        let mut used_ids = used_active;
        used_ids.extend(used_marked);
