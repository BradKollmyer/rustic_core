...
mod tests {
    ...
    use crate::{
        backend::{FileType, ReadSourceOpen, cache::Cache},
        id::Id,
    ...
    #[cfg(unix)]
    fn hold_pack_fds(n: usize) -> (tempfile::TempDir, Cache) {
        let dir = tempfile::tempdir().unwrap();
        let cache = Cache::new(RepositoryId::default(), Some(dir.path().to_path_buf())).unwrap();
        let ids: Vec<_> = (0..n).map(|_| Id::random()).collect();
        for id in &ids {
            cache
    ...
    #[cfg(unix)]
    #[test]
    fn walk_recovers_dirs_after_releasing_pack_fds() {
        if !run_in_child(
            "RUSTIC_WALK_EMFILE_RECOVER",
            "backend::ignore::tests::walk_recovers_dirs_after_releasing_pack_fds",
        ) {
            return;
        }

        let (tree, expected) = photo_tree();
        let src = local_source(tree.path());
        set_nofile(256);
        let (_cache_dir, cache) = hold_pack_fds(80);
        assert!(cache.open_file_count() > 0);
        let extra = open_until_emfile();

        let names = entry_names(&src).expect("walk should retry after dropping pack FDs");
        for name in &expected {
            assert!(names.contains(name), "missing {name} in {names:?}");
        }
        assert_eq!(
            cache.open_file_count(),
            0,
            "EMFILE retry should have dropped cached pack FDs"
        );
        drop(extra);
    }
    ...
    #[cfg(unix)]
    #[test]
    fn walk_emfile_is_fatal_when_pack_fds_cannot_help() {
        if !run_in_child(
            "RUSTIC_WALK_EMFILE_FATAL",
            "backend::ignore::tests::walk_emfile_is_fatal_when_pack_fds_cannot_help",
        ) {
            return;
        }
    ...
    #[cfg(unix)]
    #[test]
    fn open_file_retries_after_releasing_pack_fds() {
        if !run_in_child(
            "RUSTIC_OPEN_EMFILE_RETRY",
            "backend::ignore::tests::open_file_retries_after_releasing_pack_fds",
        ) {
            return;
        }
        ...
        let opened = OpenFile(file.clone())
            .open()
            .expect("open should succeed after dropping pack FDs");
        drop(opened);
